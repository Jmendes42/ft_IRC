...
#define ERR_NOSUCHNICK(channel, fd, toSend)         {toSend =  "401 " + channel + " :No such nick/channel\r\n"; send(fd, toSend.c_str(), toSend.length(), 0); return ;}
...
#define ERR_UNKNOWNMODE(channel, fd, toSend)        {toSend =  "472 " + channel + " :is unknown mode char to me\r\n"; send(fd, toSend.c_str(), toSend.length(), 0); return ;}
...
#define ERR_CHANOPRIVSNEEDED(channel, fd, toSend)   {toSend =  "482 " + channel + " :You're not channel operator\r\n"; send(fd, toSend.c_str(), toSend.length(), 0); return ;}

#define ERR_NICKNAMEINUSE(nick, fd, toSend)   {toSend =  "433 " + nick + " :Nickname is already in use\r\n"; send(fd, toSend.c_str(), toSend.length(), 0); return ;}
